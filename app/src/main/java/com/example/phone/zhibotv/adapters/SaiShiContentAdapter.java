    ...
    public void bindData(ViewHolder holder, SaiShiContenModel item, int position) {
        switch (item.getType()) {
            case 0:
                ...
                Picasso.with(context).load(item.getLeftIcon()).into(leftImage);
                Picasso.with(context).load(item.getRightIcon()).into(rightImage);
                ...
                for (int i = 0; i < item.getAnchors().size(); i++) {
                    ...
                    Picasso.with(context).load(item.getAnchors().get(i).getSourceImg()).into(imageView1);
            ...
            case 1:
                ...
                Picasso.with(context).load(item.getScheduleIcon()).into(imageView);
